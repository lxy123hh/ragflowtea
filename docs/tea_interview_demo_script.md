...
这个项目是我基于 RAGFlow 落地的茶园业务知识库 AI 问答助手。甲方提供了茶树栽培、茶文化、茶健康三类 Excel 资料，我将它们解析成 649 个知识片段、83,456 tokens 的知识库，并接入 Open WebUI 管理的本地 Ollama 模型，Chat 使用 DeepSeek-R1 70B，Embedding 使用 BGE-M3。项目中我完成了 Docker 本地部署、模型代理接入、知识库构建、问答应用配置、Open WebUI Ollama Chat 兼容性修复，以及回答质量和延迟优化。
